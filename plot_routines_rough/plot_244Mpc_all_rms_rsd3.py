...
pl.savefig('./eps/dTrms_244_rsd_b3nu44_sources.eps')
pl.savefig('./png/dTrms_244_rsd_b3nu44_sources.png')
...
leg = pl.legend(loc='lower right',prop={'size':11})
leg.draw_frame(False)
pl.savefig('./eps/dTrmsxi_244_rsd_b3nu44_sources.eps')
pl.savefig('./png/dTrmsxi_244_rsd_b3nu44_sources.png')

pl.clf()
fg = gridspec.GridSpec(3,1)
fg.update(hspace=0)
up = pl.subplot(fg[0, :])
up.plot(f1[:,1],f1[:,2],'r-',label='L1',lw=1.5)
up.plot(f2[:,1],f2[:,2],'b:',label='L2',lw=2)
up.plot(f3[:,1],f3[:,2],'g--',label='L3',lw=2)
up.plot(f4[:,1],f4[:,2],'m-.',label='L4',lw=2)
...
low.set_ylim([0,6])
low.set_xlim([210,70])
leg = low.legend(loc='lower right',prop={'size':12})
leg.draw_frame(False)
low.set_ylabel('$\langle\delta T_{\\rm b}\\rangle^{1/2}\, \\rm{[mK]}$')
low.set_xlabel('$\\nu_{\\rm obs}\, \\rm{[MHz]}$')
low.minorticks_on()

pl.setp(up.get_xticklabels(), visible=False)
pl.minorticks_on()
pl.savefig('./eps/dTmeanrms_244_rsd_b3nu44_sources.eps')
pl.savefig('./png/dTmeanrms_244_rsd_b3nu44_sources.png')
...
fig.set_xlim([210,61])
fig.set_xlabel('$\\nu_{\\rm obs}\, \\rm{[MHz]}$')
fig.set_ylabel('$\langle\delta T_{\\rm b}\\rangle^{1/2}\,\\rm{[mK]}$')
fig.minorticks_on()
...
pl.savefig('./eps/dTinset_244_rsd_b3nu44_sources.eps')
pl.savefig('./png/dTinset_244_rsd_b3nu44_sources.png')
